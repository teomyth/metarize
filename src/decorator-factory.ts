 ...
 */
export class ClassDecoratorFactory<T> extends DecoratorFactory<T, T, ClassDecorator> {
  protected mergeWithInherited(
    inheritedMetadata: T,
    target: Object,
    member?: string,
...
> {
  ...
  create(): PropertyDecorator {
    return (target: Object, propertyName: string | symbol) => this.decorate(target, propertyName);
  }
  ...
  static createDecorator<S>(
    ...
    return super._createDecorator<S, MetadataMap<S>, PropertyDecorator>(key, spec, options);
  }
}
...
> {
  private getOrInitMetadata(meta: MetadataMap<T[]>, target: Object, methodName?: string) {
    ...
    if (methodMeta == null) {
      ...
      methodMeta = new Array(DecoratorFactory.getNumberOfParameters(target, methodName)).fill(
        undefined
      );
...
> {
  ...
  private getParameterIndex(
    ...
    let index = Reflector.getOwnMetadata(`${this.key}:index`, target, methodName);
  ...
  protected mergeWithInherited(
    ...
    if (inheritedParams.length) {
      ...
      inheritedParams[index] = this.withTarget(<T>this.inherit(inheritedParams[index]), target);
    }
    // Cache the index to help us position the next parameter
    Reflector.defineMetadata(`${this.key}:index`, index - 1, target, methodName);
